        ...
        try {
            ...
            while (rs.next()) {
                if (ii.getPrice(itemId) > rs.getLong("price")) {
                    log.info("道具: " + MapleItemInformationProvider.getInstance().getName(itemId) + "道具ID: " + itemId + " 商店: " + rs.getInt("shopid") + " 价格: " + rs.getLong("price") + " 新价格:" + (long) ii.getPrice(itemId));
