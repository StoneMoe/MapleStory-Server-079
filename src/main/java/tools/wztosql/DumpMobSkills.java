...
public class DumpMobSkills {
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            log.info("Dumping mobskills");
            dq.dumpMobSkills();
            hadError |= dq.isHadError();
            currentQuest = dq.currentId();
        } catch (Exception e) {
            hadError = true;
            log.info(currentQuest + " skill.", e);
        }
        ...
        log.info("Finished" + withErrors + " in " + elapsedMinutes + " minutes " + elapsedSecs + " seconds");
    }
    ...
    public DumpMobSkills(final boolean update) throws Exception {
        ...
        this.update = update;
        this.skill = MapleDataProviderFactory.getDataProvider(Paths.get(EnvProperties.wzPath, "Skill.wz"));
        if (this.skill == null) {
            this.hadError = true;
        }
    }
    ...
    public void dumpMobSkills(final PreparedStatement ps) throws Exception {
        if (!this.update) {
            ...
            log.info("Deleted wz_mobskilldata successfully.");
        }
        final MapleData skillz = this.skill.getData("MobSkill.img");
        log.info("Adding into wz_mobskilldata.....");
        for (final MapleData ids : skillz.getChildren()) {
            for (final MapleData lvlz : ids.getChildByPath("level").getChildren()) {
                ...
                log.info("Added skill: " + this.id + " level " + lvl);
                ps.addBatch();
            }
        }
        log.info("Done wz_mobskilldata...");
    }
